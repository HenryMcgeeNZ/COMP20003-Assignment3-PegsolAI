...
struct node_s{
    int depth;
    move_t move; // the move that bought the board to this state
    state_t state;
...
node_t** free_me; // array to keep track of generated nodes to be freed
int curr_size; // current size of free_me array
int num_nodes;
...
// mallocs memory for free_me array
void initialise_free_arr();

// adds a node to free_me array
void add_to_free_arr(node_t* new);

// frees all generated nodes
void free_nodes(node_t** free_me);
